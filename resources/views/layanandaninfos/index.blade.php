...
<h2 class="text-center my-4">Data Web Bidang Layanan & Info</h2>
	<div class="d-flex justify-content-end mb-3"><a href="{{ route('layanandaninfos.create') }}" class="btn btn-info">Create</a></div>
	...
	<table class="table table-bordered">
		...
		<tbody>
			@foreach($layanandaninfos as $layanandaninfo)

				<tr>
					<td>{{ $layanandaninfo->id }}</td>
					<td>{{ $layanandaninfo->namaweb }}</td>
					<td><a href="{{ $layanandaninfo->linkweb }}">{{ $layanandaninfo->linkweb }}</a></td>
					<td>{{ $layanandaninfo->deskripsi }}</td>
					<td>{{ $layanandaninfo->status }}</td>
					<td>{{ $layanandaninfo->haripemantauan }}</td>
					<td>{{ $layanandaninfo->keterangan }}</td>

					<td>
						<div class="d-flex gap-2">
                            <a href="{{ route('layanandaninfos.show', [$layanandaninfo->id]) }}" class="btn btn-info">Show</a>
                            <a href="{{ route('layanandaninfos.edit', [$layanandaninfo->id]) }}" class="btn btn-primary">Edit</a>
                            {!! Form::open(['method' => 'DELETE','route' => ['layanandaninfos.destroy', $layanandaninfo->id]]) !!}
