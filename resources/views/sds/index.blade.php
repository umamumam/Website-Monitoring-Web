...
@section('contents')
<h2 class="text-center my-4">Data Web SD</h2>
	<div class="d-flex justify-content-end mb-3"><a href="{{ route('sds.create') }}" class="btn btn-info">Create</a></div>
		...
		<tbody>
			@foreach($sds as $sd)
				...
				<tr>
					<td>{{ $sd->id }}</td>
					<td>{{ $sd->namaweb }}</td>
					<td><a href="{{ $sd->linkweb }}">{{ $sd->linkweb }}</a></td>
					<td>{{ $sd->deskripsi }}</td>
					<td>{{ $sd->status }}</td>
					<td>{{ $sd->haripemantauan }}</td>
					<td>{{ $sd->keterangan }}</td>

					<td>
						<div class="d-flex gap-2">
                            <a href="{{ route('sds.show', [$sd->id]) }}" class="btn btn-info">Show</a>
                            <a href="{{ route('sds.edit', [$sd->id]) }}" class="btn btn-primary">Edit</a>
                            {!! Form::open(['method' => 'DELETE','route' => ['sds.destroy', $sd->id]]) !!}
